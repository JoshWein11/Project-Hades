...
GameScreen UpdateScreenSetting(GameScreen previousScreen)
{
    ...
    // --- Fullscreen Button ---
    Rectangle fullscreenBtn = { centerX - 100, 380, 200, 40 };

    // --- Back Button ---
    Rectangle backBtn = { centerX - 100, 550, 200, 40 };
...
void DrawScreenSetting(void)
{
    ...
    DrawRectangle(0, 0, VIRTUAL_WIDTH, VIRTUAL_HEIGHT, Fade(BLACK, 0.8f));

    // Title
    DrawText("SETTINGS", (int)centerX - MeasureText("SETTINGS", 40) / 2, 100, 40, RAYWHITE);

    // --- Resolution Label ---
    DrawText("Resolution", (int)centerX - MeasureText("Resolution", 24) / 2, 165, 24, LIGHTGRAY);
    ...
    DrawText("800 x 600", (int)centerX - MeasureText("800 x 600", 20) / 2, 210, 20, RAYWHITE);
    ...
    DrawText("1280 x 720", (int)centerX - MeasureText("1280 x 720", 20) / 2, 270, 20, RAYWHITE);
    ...
    DrawText("1920 x 1080", (int)centerX - MeasureText("1920 x 1080", 20) / 2, 330, 20, RAYWHITE);

    // --- Fullscreen Button ---
    Rectangle fullscreenBtn = { centerX - 100, 380, 200, 40 };
    bool hoverFS = CheckCollisionPointRec(mousePoint, fullscreenBtn);
    DrawRectangleRec(fullscreenBtn, hoverFS ? LIGHTGRAY : Fade(RAYWHITE, 0.15f));
    DrawRectangleLinesEx(fullscreenBtn, 2, RAYWHITE);
    const char* fsText = settings->isFullscreen ? "Windowed" : "Fullscreen";
    DrawText(fsText, (int)centerX - MeasureText(fsText, 20) / 2, 390, 20, RAYWHITE);
    ...
    DrawText("Volume", (int)centerX - MeasureText("Volume", 24) / 2, 445, 24, LIGHTGRAY);
    ...
    DrawText("Back", (int)centerX - MeasureText("Back", 20) / 2, 560, 20, RAYWHITE);

    // --- Hint ---
    DrawText("Press ESC to return", (int)centerX - MeasureText("Press ESC to return", 20) / 2, 610, 20, LIGHTGRAY);
}
